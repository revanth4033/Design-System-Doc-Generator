import type { VariableToken } from "./variableScanner";
import type { ColorStyleToken, TypographyStyleToken } from "./styleScanner";
import type { ComponentSetToken, ComponentToken } from "./componentScanner";
...
function createVerticalFrame(name: string, spacing: number, padding = 0): FrameNode {
...
function setSolidFill(node: GeometryMixin, hex: string): void {
...
async function buildColorsSection(colorStyles: ColorStyleToken[], variables: VariableToken[]): Promise<FrameNode> {
...
async function buildTypographySection(textStyles: TypographyStyleToken[]): Promise<FrameNode> {
...
async function buildVariablesSection(variables: VariableToken[]): Promise<FrameNode> {
  ...
  const grouped = new Map<string, VariableToken[]>();
...
async function buildComponentsSection(components: ComponentToken[], componentSets: ComponentSetToken[]): Promise<FrameNode> {
...
export async function generateDocumentationPage(data: DocumentationData): Promise<PageNode> {
